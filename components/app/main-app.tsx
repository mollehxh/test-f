...
export function MainApp({ onOpenSettings }: { onOpenSettings: () => void }) {
  const [factories, setFactories] = React.useState<Factory[]>(INITIAL_FACTORIES)
  const [activeId, setActiveId] = React.useState<string>(
    INITIAL_FACTORIES[0]?.id ?? "",
  )

  const activeFactory = factories.find((factory) => factory.id === activeId) ?? null

  const handleRename = (id: string, name: string) => {
    setFactories((prev) =>
      prev.map((factory) => (factory.id === id ? { ...factory, name } : factory)),
    )
  }
  ...
  const handleAdd = () => {
    ...
    setFactories((prev) => [
      ...prev,
      { id, name: `Новый завод ${prev.length + 1}` },
    ])
    setActiveId(id)
  }
